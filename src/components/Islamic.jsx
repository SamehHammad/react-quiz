...
const Islamic = () => {
  ...
  const finalopation = (isCorrect) => {
    ...
    if (currentquestion + 1 < questionsIslamic.length) {
  ...
  const helpStyle = () => {
    showCorrectAns(currentquestion);
  };
  const congrat = () => {
    theEnd();
  };
  return (
    <div className="App ">
      <ProgressBar
        variant="success"
        now={((currentquestion + 1) * 100) / questionsIslamic.length}
      />
      <div className="welc">
        <h1 className="header">الإختبار الاسلامي</h1>
      ...
      {finalreuslt ? (
        <div onLoad={congrat()} className="final-result">
          <h1>نتيجتك</h1>
          <h2 className="result">
            ({(score / questionsIslamic.length) * 100}%)
          </h2>
          <div>
            {score > 7 ? (
        ...
        <div className="Question-Card">
          <h2 className="title">
            السؤال {currentquestion + 1} من {questionsIslamic.length}
          </h2>
          <Button variant="success" onClick={() => helpStyle()}>
            Help
          </Button>
          <h3 className="Question-text">
            {questionsIslamic[currentquestion].text}
          </h3>

          {questionsIslamic[currentquestion].options.map((quis) => (
            <ul key={quis.id}>
              <li
